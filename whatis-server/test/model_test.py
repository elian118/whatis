...
from consts.paths import LOCAL_MODEL_PATH

# 1. 테스트에 사용할 이미지 준비 (로컬 파일이나 웹에서 임시 다운로드)
...
# 4. 예측 결과 확인 (가장 높은 점수의 클래스 인덱스 찾기)
predicted_class_idx = logits.argmax(-1).item()

# 5. 클래스 ID를 레이블(이름)로 변환
